(function () {
  ...
  function updateBookingSummary() {
    ...
    lineLabel.textContent =
      formatCurrency(TOUR_PRICE) + " x " + totalGuests + " khach";
  ...
  if (shareButton) {
    shareButton.addEventListener("click", async function () {
      ...
      showToast(
        copied ? "Da sao chep lien ket tour." : "Khong the sao chep lien ket.",
      );
    });
  }
  ...
  if (directionsButton) {
    directionsButton.addEventListener("click", function () {
      window.open(
        "https://www.google.com/maps/search/?api=1&query=Vinh+Ha+Long%2C+Quang+Ninh",
        "_blank",
        "noopener",
      );
    });
  ...
  if (saveLocationButton) {
    saveLocationButton.addEventListener("click", async function () {
      ...
      showToast(
        copied ? "Da luu thong tin dia diem." : "Khong the luu dia diem.",
      );
    });
  }
