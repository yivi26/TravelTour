(function () {
  ...
  async function loadBookingDetailIfExists() {
    const bookingId = getQueryParam("booking_id");

    if (!bookingId) {
      return false; // không phải từ booking → dùng logic cũ
    }
    ...
    try {
      ...
      renderBookingDetailToTourPage(result.data);
      return true;
    } catch (err) {
      console.error(err);
      return true;
  ...
  function renderBookingDetailToTourPage(data) {
    document.getElementById("tour-title").innerText = data.tour_name;

    document.getElementById("tour-location").innerText = data.location;

    document.getElementById("tour-price").innerText =
      Number(data.final_price).toLocaleString("vi-VN") + " VNĐ";

    document.getElementById("booking-tour-price").innerText =
      Number(data.final_price).toLocaleString("vi-VN") + " VNĐ";
  }
  ...
  async function fetchTourDetail(id) {
    const response = await fetch(
      `http://localhost:3000/api/provider/public/tours/${id}`,
    );
  ...
  function renderPolicySection(tour) {
    ...
    const termsConditionsWrap = document.getElementById(
  ...
  function renderTourDetail(tour) {
    ...
    const tourDescriptionFull = document.getElementById(
    ...
    renderList(
      "tour-excludes-list",
      excludes,
      "Chưa có thông tin không bao gồm",
    );
  ...
  function renderExtraInfo(tour) {
    ...
    const hasTransport =
      tour.transport_info && tour.transport_info.trim() !== "";
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    const isBooking = await loadBookingDetailIfExists();

    if (!isBooking) {
      init(); // load tour bình thường
    }
  });
})();
